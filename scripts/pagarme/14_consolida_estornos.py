...
def consolidar_estornos_pagarme():
    """
    Consolida múltiplos arquivos de estornos Pagar.me em único CSV

    Entrada: PAGAR.ME_ESTORNOS_[1-2].xlsx
    Saída: PAGAR.ME_ESTORNO_CONSOLIDADO.csv
    """
    base_temp = PATHS['temp']

    files = [
        os.path.join(base_temp, 'PAGAR.ME_ESTORNOS_1.xlsx'),
        os.path.join(base_temp, 'PAGAR.ME_ESTORNOS_2.xlsx')
    ]
    ...
    if 'Refunded_Amount' in merged_df.columns:
        merged_df['Valor Estornado'] = merged_df['Refunded_Amount']
    else:
        merged_df = converter_centavos_para_reais(
            merged_df,
            ['Amount_In_Cents']
        )
        merged_df['Valor Estornado'] = merged_df['Amount_In_Cents']
    ...
    selected_columns = [
        ...
        'Número de Parcelas', 'Valor Estornado', 'Status'
    ]

    output_file = os.path.join(base_temp, 'PAGAR.ME_ESTORNO_CONSOLIDADO.csv')
...
if __name__ == '__main__':
    print("=== Consolidação de Estornos Pagar.me ===")
    consolidar_estornos_pagarme()
    print("Concluído!\n")
